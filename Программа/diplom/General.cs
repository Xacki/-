...
namespace diplom
{
    public partial class General : Form
    {
        ...
        private void show(string nameFolder,DataRowView curGroup,DataRowView curStud) 
        {
            ...
            string folderSavePriloj1;
            string folderSavePriloj2;
            string folderSavePriloj3;

            folderSavePriloj1 = nameFolder + "\\" + FIO + "\\Приложения\\Научно-исследовательская деятельность\\";
            Directory.CreateDirectory(folderSavePriloj1);

            folderSavePriloj2 = nameFolder + "\\" + FIO + "\\Приложения\\Внеаудиторная деятельность\\";
            Directory.CreateDirectory(folderSavePriloj2);

            folderSavePriloj3 = nameFolder + "\\" + FIO + "\\Приложения\\Дополнительное образование, самообразование\\";
            Directory.CreateDirectory(folderSavePriloj3);
            ...
            foreach (DataRowView row in nauRab2BS)
            {
                if (row["idStud"].ToString() == curStud["id"].ToString())
                {
                    if (row["foto"].ToString() != "")
                    {
                        ...
                        memStream.Position = 0;
                        img = Image.FromStream(memStream);
                        img.Save(folderSavePriloj1 + row["rez"].ToString() + row["fotoF"].ToString(), System.Drawing.Imaging.ImageFormat.Jpeg);
                    }
                }
            }
            ...
            foreach (DataRowView row in vneRab3BS)
            {
                if (row["idStud"].ToString() == curStud["id"].ToString())
                {
                    if (row["foto"].ToString() != "")
                    {
                        ...
                        memStream.Position = 0;
                        img = Image.FromStream(memStream);
                        img.Save(folderSavePriloj2 + row["rez"].ToString() + row["fotoF"].ToString(), System.Drawing.Imaging.ImageFormat.Jpeg);
                    }
                }
            }
            ...
            foreach (DataRowView row in dopRab4BS)
            {
                if (row["idStud"].ToString() == curStud["id"].ToString())
                {
                    if (row["foto"].ToString() != "")
                    {
                        ...
                        memStream.Position = 0;
                        img = Image.FromStream(memStream);
                        img.Save(folderSavePriloj3 + row["rez"].ToString() + row["fotoF"].ToString(), System.Drawing.Imaging.ImageFormat.Jpeg);
                    }
                }
            }
        }
        ...
        private void general_KeyUp(object sender, KeyEventArgs e)
        {
                if (e.KeyCode == Keys.Return)
                {
                    ...
                    kursBS_CurrentChanged(sender, e);
                    praktBS_CurrentChanged(sender, e);
                }
        }
        ...
        private void kursBS_CurrentChanged(object sender, EventArgs e)
        {
            if (portfolioDS.kurs.GetChanges() != null)
            {
                Validate();
                kursBS.EndEdit();
                kursTA.Update(portfolioDS.kurs);
        ...
        private void praktBS_CurrentChanged(object sender, EventArgs e)
        {
            if (portfolioDS.prakt.GetChanges() != null)
            {
                Validate();
                praktBS.EndEdit();
                praktTA.Update(portfolioDS.prakt);
